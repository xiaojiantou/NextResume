  ...
  return (
    <div className="flex items-center gap-2" ref={ref}>
      <div className="relative">
        ...
        {open && (
          <div className="absolute right-0 top-full mt-1.5 z-20 w-80 card p-1.5 shadow-pop max-h-[70vh] overflow-y-auto">
            {(["novita", "openai", "anthropic", "gemini"] as const).map(
              (provider) => {
                const group = MODELS.filter((m) => m.provider === provider);
                if (group.length === 0) return null;
                return (
                  <div key={provider}>
                    <div className="px-2 pt-2 pb-1 text-[10px] uppercase tracking-widest text-ink-400 font-medium">
