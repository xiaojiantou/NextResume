...
function detectColumnSplit(
  ...
  // The corridor test below reads each run's ink interval, so a file that
  // reports no widths leaves it nothing to measure. Fragmentation itself is
  // fine — per-character runs still tile their line — but unmeasured runs are
  // handed to the visual check rather than guessed at.
  const measured = body.filter((item) => item.width > 0).length;
  if (measured / body.length < 0.5) return { split: null, confident: false };
  ...
  if (bands.length < 8) return { split: null, confident: false };
  ...
  // Legible geometry showing no corridor is a confident single column, which
  // is what lets the caller skip the visual check. That shortcut is only safe
  // because the corridor test cannot be fooled by inline bold — the heuristic
  // it replaced reported "confidently two columns" for an ordinary
  // single-column resume whose keywords happened to be bold.
  return { split: best?.split ?? null, confident: true };
}
