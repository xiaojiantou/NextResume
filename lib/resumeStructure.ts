...
// Bumped whenever the rewrite pipeline changes what it would produce for the
// same input. Stamped on every optimization and folded into the variant cache
// key, so a result generated by an older pipeline is regenerated instead of
// shown as if it were current.
export const OPTIMIZATION_PIPELINE_VERSION = 8;
  ...
  return smallHash(
    JSON.stringify(
      stableValue({
        ...
        contentQualityVersion: OPTIMIZATION_PIPELINE_VERSION,
      }),
    ),
  );
}
