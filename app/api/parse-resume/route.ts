...
- If one company/role contains nested team, org, group, department, product-area, or project headings, keep one company entry and put each nested heading in "teams"; put that team's achievements under that team. Do not split the same company into duplicate experience entries just because the source lists teams.
- A line inside a role that names a project or product and carries its own date range (e.g. "IOLTA Ledger: AI-Powered Trust Accounting Jul 2026 - Present") followed by indented or dash-prefixed lines is such a nested heading: put the name in "teams[].name", any text after a colon in "title", the dates in "start"/"end", and the lines under it in that team's "bullets". It is NEVER itself a bullet. Strip leading list markers such as "–" or "•" from bullet text.
