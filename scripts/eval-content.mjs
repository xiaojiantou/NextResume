...
import { reviewContentQuality, parseContentReviews, CONTENT_REVIEW_SYSTEM, contentRevisionIssues } from '../lib/contentQuality.ts';
...
const casesPath = option('--cases', 'eval/content/cases.json');
const outputPrefix = option('--out-prefix', 'eval/content');
const requestedBaseline = option('--baseline-ref', null);
const baselineRef = requestedBaseline ? execFileSync('git', ['rev-parse', '--verify', requestedBaseline], { encoding: 'utf8' }).trim() : null;
const cases = JSON.parse(readFileSync(casesPath, 'utf8'));
const disagreement = (c, review) => (review?.status === 'improved' ? 'improved' : 'retain') !== c.expected || review?.status === 'unreviewed' || (c.needsEvidence && !review?.question) || (c.expectedNextStep && review?.nextStep !== c.expectedNextStep) || (c.expectedMetric && !review?.impactMetrics?.includes(c.expectedMetric));
...
if (process.argv.includes('--calibrate')) {
  ...
  const disagreements = cases.filter(c => disagreement(c, reviews.get(c.id))).map(c => c.id);
  writeFileSync(`${outputPrefix}/calibration.json`, JSON.stringify({ at: new Date().toISOString(), model, disagreements, reviews: Object.fromEntries(reviews), calls }, null, 2) + '\n');
...
const baselineQualityPath = resolve(`lib/.content-quality-baseline-${process.pid}.ts`);
if (baselineRef) {
  const previousChunks = execFileSync('git', ['show', `${baselineRef}:lib/optimizeChunks.ts`], { encoding: 'utf8' });
  const previousQuality = execFileSync('git', ['show', `${baselineRef}:lib/contentQuality.ts`], { encoding: 'utf8' });
  writeFileSync(baselineQualityPath, previousQuality);
  writeFileSync(baselinePath, previousChunks.replace('./contentQuality.ts', `./.content-quality-baseline-${process.pid}.ts`));
} else writeFileSync(baselinePath, readFileSync('eval/content/baseline-prompts.txt'));
try {
  ...
  const rows = await mapWithConcurrency(cases, 1, async c => {
    ...
    const revisions = contentRevisionIssues([{ id: c.id, source: c.source, candidate: revisedText }], new Map([[c.id, revisedReview]]), new Set());
    if (revisions.length) {
      after = await complete(buildChunkPrompt({ ...args, feedback: revisions.map(item => item.issue) }), 'revision');
    ...
    completed.push(row);
    writeFileSync(`${outputPrefix}/partial.json`, JSON.stringify({ model, completed, calls }, null, 2) + '\n');
    return row;
  });
  ...
  const disagreements = cases.filter(c => disagreement(c, calibration.get(c.id))).map(c => c.id);
  const output = { casesPath, baselineRef, at: new Date().toISOString(), model, corpus: 'authored regression cases, not customer resumes', scope: 'writing + quality selection; full route safety validators covered separately by tests', disagreements, calibration: Object.fromEntries(calibration), calls, rows };
  writeFileSync(`${outputPrefix}/latest.json`, JSON.stringify(output, null, 2) + '\n');
  console.log(`Saved ${outputPrefix}/latest.json; ${calls.length} calls; calibration disagreements: ${disagreements.join(', ') || 'none'}. Human preferences remain unfilled.`);
  unlinkSync(`${outputPrefix}/partial.json`);
  if (disagreements.length) process.exitCode = 1;
} finally { unlinkSync(baselinePath); if (baselineRef) unlinkSync(baselineQualityPath); }
